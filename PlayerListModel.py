...
class PlayerListModel(QAbstractListModel):
    ...
    def addPlayer(self, id: int, name: str):
        ...
        self._players.append({'id': id, 'name': name})
        self.endInsertRows()

    @pyqtSlot(int, int, str)
    def insertPlayer(self, row: int, id: int, name: str):
        self.beginInsertRows(QModelIndex(), row, row)
        self._players.insert(row, {'id': id, 'name': name})
        self.endInsertRows()

    @pyqtSlot(int, str, int)
    def editPlayer(self, row: int, id: int, name: str):
        ix = self.index(row, 0)
        self._players[row] = {'id': id, 'name': name}
